...
using mutex_t = jh::sync::ipc::process_mutex<"test_mutex">;
...
TEST_CASE("process_mutex basic lock/unlock") {
    ...
    jh::sync::ipc::process_mutex<"test_mutex", true>::unlink();
}

// ---- Cross-process tests ----

/// writer and reader are launched as external processes
using writer_launcher = jh::sync::ipc::process_launcher<"../examples/process_lock/writer">;
using reader_launcher = jh::sync::ipc::process_launcher<"../examples/process_lock/reader">;
...
TEST_CASE("process_launcher runs writer and reader") {
...

    {
        ...
        // The successful timed-lock result is part of the process behavior;
        // the elapsed time is intentionally not asserted here.
    }
